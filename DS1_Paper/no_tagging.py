...
# Logging setup (from all scripts)
...
# Text extraction functions (from script1)
...
# LLM interaction function (adapted from script1 and script2)
...
# JSON parsing function (from script2)
...
# Process documents and generate labels directly (combined logic from script1 and script2)
def process_documents_and_generate_labels(
    folder_path: str, 
    ...
    """
    Process documents from a folder and directly generate topic labels.
    Returns the dictionary mapping labels to document names.
    """
    ...
    logger.info("Building prompt for label generation...")
    papers_info = []
    for doc_name, text in doc_texts.items():
        ...
        papers_info.append(f"Paper Name: {doc_name}\nContent:\n{truncated_text}\n")

    joined_papers_text = "\n".join(papers_info)

    # The instruction prompt
    prompt = f"""
We have the following papers with their extracted content. Please group them together by generating labels that reflect common themes.
...
{{
  ...
  "label2": ["PaperC.pdf"]
    ...
    # Send prompt to LLM
    logger.info("Sending documents to LLM for label generation...")
    ...
    # Parse the output
    labels_dict = parse_llm_json(llm_output, logger)
    
    # Save the result
    logger.info(f"Saving generated labels to {output_json_path}")
    try:
    ...
    return labels_dict, doc_texts

# Metric computation functions (from script4)
...
def main():
    ...
    LABELS_JSON_PATH = r'DS1_Paper\no-tagging_generated_labels.json'
    EVAL_OUTPUT_JSON = r'DS1_Paper\no-tagging_evaluation_results.json'
    MODEL = 'gpt-4o'  
    
    # Setup logging
    logger = setup_logging()
    logger.info("Starting document labeling and evaluation process")
    
    try:
        # Process documents and generate labels
        labels_dict, papers_text = process_documents_and_generate_labels(
            folder_path=FOLDER_PATH,
            output_json_path=LABELS_JSON_PATH,
            logger=logger,
            model=MODEL
        )
        
        # Vectorize texts for evaluation
        ...
        logger.info(f"Saving evaluation results to {EVAL_OUTPUT_JSON}")
