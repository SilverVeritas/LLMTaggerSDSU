...
def setup_logging(log_dir="logs") -> logging.Logger:
    ...
    logger = logging.getLogger('multilabel_metrics')
    logger.setLevel(logging.DEBUG)
    fn = os.path.join(
        log_dir,
        f'multilabel_metrics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    )
...
def load_multilabel_model_results(file_path: str, logger: logging.Logger):
    """
    Load multi-label model results and convert to label_dict format
    """
    logger.info(f"Loading multi-label model results from {file_path}")
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            model_results = json.load(f)
        
        # For traditional model, the format is already {label: [papers]}
        # but might have additional metadata to handle
        if isinstance(next(iter(model_results.values()), None), dict):
            # Convert format to {label_name: [doc1, doc2, ...]}
            labels_dict = {}
            for topic_label, info in model_results.items():
                # Extract label name for readability
                clean_label = topic_label.split(':', 1)[1].strip() if ':' in topic_label else topic_label
                labels_dict[clean_label] = info["papers"]
            return labels_dict
        else:
            # Simple format {label: [papers]} - return as is
...
def main():
    LABELS_JSON_PATH = r"DS2_APT/trad-1-multilabel_bertopic_results.json"
    PDF_FOLDER = r"DS2_APT/APT2015"
    EVAL_OUTPUT_JSON = r"DS2_APT/trad-2-evaluation_results.json"

    logger = setup_logging()

    try:
        # Load the labels
        labels_dict = load_multilabel_model_results(LABELS_JSON_PATH, logger)
